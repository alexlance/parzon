...
class ZoneFile(object):
    ...
    def replace_at_with_origin(self, text):
        ...
        if not self.origin:
            ...
            sys.exit(1)
        return re.sub('^@\s+', self.origin+' ', text, flags=re.MULTILINE)
    ...
    def condense_parenthesis(self, text):
        ...
        """
        condense_brackets = re.compile('\((.*?)\)', re.DOTALL)
    ...
    def get_resource_records(self, text):
        ...
        for line in text.split("\n"):
            ...
            if line[0] in [" ", "\t"] and re.search("\w+", line[1:]) and len(current_name):
